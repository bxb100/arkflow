...
//! Integration tests for Kafka input with codec support

use arkflow_core::config::EngineConfig;

#[test]
fn test_kafka_input_with_json_codec_config() {
    let yaml = r#"
logging:
  level: info
...
"#;
    ...
    // Check that InputConfig has codec field set
...
#[test]
fn test_kafka_input_without_codec_config() {
    let yaml = r#"
logging:
  level: info
...
"#;
    ...
    // Verify codec is None when not configured
    assert!(stream_config.input.codec.is_none());
}

#[test]
fn test_generate_input_with_codec_config() {
    let yaml = r#"
logging:
  level: info
